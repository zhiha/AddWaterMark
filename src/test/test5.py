...
def create_watermark(content):
    ...
    # c.drawString(2 * cm, -0.8 * cm, content)
    # c.drawString(2 * cm, -0.8 * 2 * cm, content)
    #
    # c.drawString(12 * cm, 0 * cm, content)
    # c.drawString(12 * cm, -0.8 * cm, content)
    # c.drawString(12 * cm, -0.8 * 2 * cm, content)
    space = -0.825
    ...
    # for i in range(-3, 1):
    #     for j in range(-2, 4):
    #         c.drawString(60+(i * 8) * cm, -4.5+(j * 1.6) * cm, content)
    c.save()  # 关闭并保存pdf文件
...
def main():
    ...
    for person in persons:
        ...
        ### 加权限
        unmeta = PdfFileReader(output_pdf)
